...
const SFSingleArticle = (props: SFSingleArticleProps) => {
  ...
  return (
    <Col xs={12} data-bs-theme="dark">
      <Card className="border-0 bg-black bg-opacity-25 overflow-hidden rounded-4" style={{ boxShadow: "0px 0px 0.5em 0em white" }}>
        <Row className="g-0">
          <Col lg={4}>
            <div
              style={{
          ...
          <Col lg={8}>
            <Card.Body className="h-100 d-flex flex-column">
              <div className="gap-lg-3 d-flex flex-column flex-lg-row justify-content-between align-items-lg-center border-bottom border-light border-opacity-10 pb-lg-3">
                ...
                <p className="text-end mb-0 text-nowrap text-secondary-emphasis align-self-end">{new Date(props.article.published_at).toDateString()}</p>
              </div>

              <Card.Text className="flex-grow-1 align-content-center pt-3 pt-lg-0">{props.article.summary}...</Card.Text>
            </Card.Body>
          </Col>
        </Row>
      </Card>
